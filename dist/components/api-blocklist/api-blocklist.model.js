"use strict";
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __importDefault = (this && this.__importDefault) || function (mod) {
    return (mod && mod.__esModule) ? mod : { "default": mod };
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.ApiBlockListModel = void 0;
const logger_1 = __importDefault(require("../../lib/utils/logger"));
const api_blocklist_schema_1 = require("./api-blocklist.schema");
const logFileName = "[ApiBlocklistModel]";
class ApiBlockListModel {
    addApiToBlockList(deviceId, apiDetails) {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                const newApiBlockBody = { token: apiDetails.token, deviceId, expiresOn: apiDetails.expiresOn, createdOn: apiDetails.createdOn, blockedOn: new Date() };
                const newApiBlock = new api_blocklist_schema_1.ApiBlockList(newApiBlockBody);
                yield newApiBlock.addToList();
                logger_1.default.info(logFileName, apiDetails);
            }
            catch (e) {
                logger_1.default.debug(e);
                throw new Error(e.message);
            }
        });
    }
}
exports.ApiBlockListModel = ApiBlockListModel;
exports.default = new ApiBlockListModel();
//# sourceMappingURL=api-blocklist.model.js.map
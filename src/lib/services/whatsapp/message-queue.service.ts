import { Message } from './../../../components/messages/message.schema';
...
export class MessageQueue {
    public async getPendingsMessages(limit: number = 10) {
        const pendingMessages = await Message.find({ status: "pending" }).sort({ _id: 1 }).limit(limit);
    ...
    private  updateMessage = async(id:string)=>{
        await Message.updateOne({ _id: id }, { status: 'sent' });
    }
